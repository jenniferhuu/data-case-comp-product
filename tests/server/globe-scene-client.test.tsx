...
describe('GlobeScene client state', () => {
  ...
  it('keeps donor recipient outlines visible after selecting one recipient country', async () => {
    useDashboardState.setState({
      ...parseDashboardQuery(),
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.includes('/api/globe?') && url.includes('donor=gates-foundation') && !url.includes('recipientCountry=')) {
        return {
          ok: true,
          json: async () => ({
            arcs: [
  ...
  it('switches non-recipient donor-country clicks while a donor is selected', async () => {
    useDashboardState.setState({
      ...parseDashboardQuery(),
      selectionType: 'donor',
      selectionId: 'gates-foundation',
